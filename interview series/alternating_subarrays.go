 ...
 */
 ...
 func checkLeft(index int,k int,A []int) bool {
     currentBit:=A[index]
     for i:=index-1; k!=0 && i>=0;k,i=k-1,i-1{
         if currentBit==A[i]{
             return false
         }
         currentBit=A[i]
 ...
 func checkRight(index int,k int,A []int) bool {
     currentBit:=A[index]
     n:=len(A)
     for i:=index+1;k!=0 && i<n;k,i=k-1,i+1{
         if currentBit==A[i]{
             return false
         }
         currentBit=A[i]
...
func solve(A []int , B int )  ([]int) {
    ...
    answer:= make([]int,0)
    for i:=B; i<(n-B); i=i+1 {
        if checkLeft(i,B,A) && checkRight(i,B,A){
